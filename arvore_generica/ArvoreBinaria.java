public class ArvoreBinaria <T extends Comparable<T>> { //Ela está parametrizada com um tipo qualquer <T>
    private No<T> raiz;
    ...
    public void insere (T info) {
        No<T> novo = new No<>(info); //Como ela é a mesma classe, o tipo <> pode ser omitido
        if (arvoreVazia())
            raiz = novo;
        else
            insereRec(novo, raiz);
